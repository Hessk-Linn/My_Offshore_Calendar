...
export default function AnnualProjection({ year }) {
  ...
  const ms = Array.from({ length: 12 }, (_, i) => getMonthStats(new Date(year, i, 1), anchorDate, travelDays, r, rotationOn, rotationOff, mmkMonthlyAmount));
  ...
  return (
    <div className="bg-[#0f172a] border border-slate-800 rounded-xl p-4">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div className="p-3 bg-slate-800/50 rounded-lg"><p className="text-slate-500 text-[10px] uppercase font-bold">Work / Travel Days</p><p className="text-lg font-bold text-slate-100">{aW} <span className="text-slate-500">/</span> {aTr}</p></div>
      </div>
